...
> Мультиагентная система для анализа IT-рынка труда на базе локальных LLM (Ollama | qwen2.5:7B).  
> Вводишь название должности — получаешь полный карьерный отчёт: карту навыков, зарплатные вилки, план развития и оценку
> качества.
...
Проект представляет собой **пайплайн из 4-х специализированных AI-агентов**, которые последовательно обрабатывают запрос
и передают результат друг другу. Каждый агент работает с локальной LLM через Ollama, а ответы валидируются по строгим
Pydantic-схемам с помощью Instructor.
...
- [Описание](#описание) — идея и перечень функционала
...
│  Аналитик рынка     │──► skill_map (языки, фреймворки, инфраструктура, soft-skills)
...
│                     │    + gap_analysis (навыки, которые нужно освоить) + идея portfolio-проекта
└─────────────────────┘
        │
        ▼
┌─────────────────────┐
...
- `learning_path`: 3 фазы — Foundation → Practice → Portfolio, каждая с темами и реальными ресурсами (курсы, книги,
  документация)
...
#### 1. Установите Ollama и скачайте модель. Можно перейти на шаг 5 и воспользоваться API_KEY любого провайдера.
...
### Примеры отчётов в examples

---

* #### <a href="https://github.com/Raisin228/ml_agentic_system_analyzer_job_market/blob/main/examples/python_dev/python_backend_dev.md">Отчёт по Python Backend Dev</a>

* #### <a href="https://github.com/Raisin228/ml_agentic_system_analyzer_job_market/blob/main/examples/ml_engineer/ml_engineer.md">Отчёт по ML Engineer</a>

* #### <a href="https://github.com/Raisin228/ml_agentic_system_analyzer_job_market/blob/main/examples/ios_dev/ios_dev.md">Отчёт по IOS Dev</a>
